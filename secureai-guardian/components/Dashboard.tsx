        ...
        {[
          ...
          <div key={i} className={`bg-white/[0.03] backdrop-blur-2xl border ${isPowerUser ? 'border-red-500/20' : 'border-white/10'} p-4 sm:p-6 md:p-8 lg:p-10 rounded-2xl sm:rounded-[2.5rem] shadow-2xl hover:bg-white/[0.06] transition-all group cursor-default border-t-white/20`}>
            ...
            <div className="flex items-end gap-3">
              <span className={`${stat.val.length > 8 ? 'text-xl sm:text-2xl md:text-3xl' : 'text-2xl sm:text-3xl md:text-4xl'} font-black ${stat.color} whitespace-nowrap`}>{stat.val}</span>
              <span className="text-[8px] sm:text-[9px] md:text-[10px] font-mono font-black text-gray-600 uppercase tracking-tighter flex-shrink-0 ml-auto">{stat.sub}</span>
            </div>
          </div>
        ))}
      </div>
