# Cowork Prompt — SBI Training Run #2 (Phase 4)

This is the second Kaggle training run. The dataset `secureai-celeb-df-v2-frames`
is already uploaded from the first run, so we can skip the upload step.
...
You are driving a browser to launch a second Kaggle GPU training run — this one
uses Self-Blended Images (SBI) augmentation. The dataset from the first run is
still on Kaggle and will be reused. When you hit anything you cannot do yourself
(login, 2FA, SMS, captcha), STOP and tell me exactly what action to take, then
wait for me to say 'done' before continuing.

## Context

- Project: SecureAI DeepFake Detection
- First training run: COMPLETE (best_model.pth, test AUC 0.9065)
- This run: SBI augmentation technique — expected to improve cross-dataset generalization
- Dataset on Kaggle: secureai-celeb-df-v2-frames (already uploaded, do NOT re-upload)
- Final deliverable: download best_sbi_model.pth (~90 MB) and save to
  C:\Users\ssham\OneDrive\New Business - SecureAI\DeepFake Detection Model\SecureAI-DeepFake-Detection\ai_model\trained_models\resnet50_sbi.pth

## Step 1 — Open Kaggle

1. Go to https://www.kaggle.com
2. Confirm I'm signed in (my avatar is top-right). If not, stop and ask me to sign in.
...
3. Right sidebar, Settings:
   - Accelerator: **GPU T4 x2** (phone verification should already be done from run #1)
   - Internet: ON
4. Right sidebar, Input / Add Data:
   - Search "secureai-celeb-df-v2-frames" under Your Datasets
   - Click "+" to add it
   - Verify the input tree shows `celeb_df_v2_frames/`

## Step 3 — Paste the SBI training code

Paste the ENTIRE code block from the "SBI TRAINING CODE" section at the bottom
of this prompt into the first code cell.

## Step 4 — Run the notebook

1. Click Run All (▶▶ or menu → Run All)
2. First few outputs to verify:
   - "Device: cuda" (not cpu)
   - "GPU: Tesla T4" or similar
   - "SBI train pool: 18,121 real frames"
   - "/kaggle/input/secureai-celeb-df-v2-frames/celeb_df_v2_frames/val: 6,390 frames"
   - "/kaggle/input/secureai-celeb-df-v2-frames/celeb_df_v2_frames/test: 5,180 frames"
3. If anything doesn't match, STOP and tell me the actual output.

## Step 5 — Wait for training (~60-90 minutes)

Each epoch prints a line like:
  Epoch  12/30  TrainLoss: 0.1234  ValLoss: 0.1456  ValAUC: 0.9123  ValAcc: 87.45%  Time: 120s

Training runs 30 epochs. Watch for any ValAUC above 0.93 — that would indicate
SBI generalization is working.

If the notebook crashes or Kaggle times out, capture the error and report to me.

## Step 6 — Download the trained model

When you see "SBI-TRAINED RESNET50 — FINAL TEST RESULTS", training is done.

1. In the right sidebar's /kaggle/working/ file tree, find:
   - best_sbi_model.pth (~90 MB)
   - sbi_test_results.json (~2 KB)
2. Download both files (right-click → Download, or click file then Download icon)
3. They'll land in C:\Users\ssham\Downloads\

## Step 7 — Move files to the repo

1. Move Downloads\best_sbi_model.pth
   to ai_model\trained_models\resnet50_sbi.pth
2. Move Downloads\sbi_test_results.json
   to ai_model\trained_models\sbi_test_results.json
3. If you can't do the file move yourself, tell me:
   "Please move best_sbi_model.pth and sbi_test_results.json from Downloads to
    ai_model/trained_models/ in the repo (with the rename), then say 'done'."
...
Compared to the first run's test AUC of 0.9065:
  Delta AUC: +x.xxxx (or -x.xxxx)

Model saved to: ai_model/trained_models/resnet50_sbi.pth
Results:        ai_model/trained_models/sbi_test_results.json
Kaggle notebook URL: https://www.kaggle.com/code/<url>
```

Fill in actual numbers from sbi_test_results.json.

## Escalation Rules

Any of these → STOP and tell me what to do:
- Login / password prompt
- 2FA / SMS / captcha
- GPU quota exceeded (rare — we have 30 hrs/week)
- Email verification
- Any unexpected dialog

Format each escalation as:
  STOP: <one-line description>
  ACTION: <what I should click or type>
...
for epoch in range(1, EPOCHS+1):
    ...
    print(f"Epoch {epoch:3d}/{EPOCHS}  TrainLoss: {tr_loss/len(train_loader):.4f}  "
          ...
          f"ValAcc: {val_acc*100:.2f}%  Time: {elapsed:.0f}s")
...
4. Stay nearby for the first ~5 minutes (just in case Kaggle asks anything).
5. Training runs ~60-90 min. Cowork can minimize and check back.
6. When cowork reports "SBI TRAINING COMPLETE", you're done.
