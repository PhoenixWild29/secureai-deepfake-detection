/**
 * ProgressiveVideoUploader Component
 ...
 * @param {Function} props.onAnalysisStart - Callback when analysis starts
 * @param {Function} props.onDuplicateDetected - Callback when duplicate is detected
 * @param {Function} props.onError - Callback when error occurs
 * @param {Object} props.options - Upload options
 * @param {boolean} props.autoTransition - Whether to auto-transition to analysis tracker
 * @param {string} props.className - Additional CSS classes
 * @returns {JSX.Element} Progressive video uploader component
 */
const ProgressiveVideoUploader = ({
  onUploadComplete,
  onAnalysisStart,
  onDuplicateDetected,
  onError,
   ...
   */
  const handleFileSelect = useCallback(async (acceptedFiles, rejectedFiles) => {
   ...
   * Extract file metadata
   */
  const extractFileMetadata = useCallback(async (file) => {
    return {
      name: file.name,
      size: file.size,
      type: file.type,
  ...
  const checkForDuplicate = useCallback(async (file) => {
    try {
      ...
      return { isDuplicate: false, hash: null, error };
    }
  }, []);
   ...
   */
  const estimateUploadTime = useCallback((fileSize) => {
